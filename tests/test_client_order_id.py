...
class TestGetNewClientOrderId:
    ...
    def test_client_order_id_without_prefix(self, mocker, trading_pair):
        """Test that client order ID is generated without prefix when not provided."""
        mocker.patch("connector_utils.client_order_id.get_tracking_nonce", return_value=1640001112223334)
    ...
    def test_client_order_id_with_prefix(self, mocker, trading_pair):
    ...
    def test_client_order_id_respects_max_length(self, mocker, trading_pair):
        """Test that client order ID respects max_id_len parameter."""
        mocker.patch("connector_utils.client_order_id.get_tracking_nonce", return_value=1640001112223334)

        full_id = get_new_client_order_id(is_buy=True, trading_pair=trading_pair)
    ...
    def test_client_order_id_with_buy_side(self, mocker, trading_pair):
    ...
    def test_client_order_id_with_sell_side(self, mocker, trading_pair):
    ...
    def test_client_order_id_with_long_prefix_and_max_len_uses_full_suffix(self, mocker, trading_pair):
        """Test that when max_id_len is large enough, full suffix is included."""
        nonce_value = 1640001112223334
        mocker.patch("connector_utils.client_order_id.get_tracking_nonce", return_value=nonce_value)
        ...
        shortened_id = get_new_client_order_id(
            ...
            trading_pair=trading_pair,
            hbot_order_id_prefix=prefix,
            max_id_len=len(prefix) + 5 + 13 + 5,  # prefix + side + symbols + time hex + instance id start
        )
    ...
    def test_client_order_id_with_max_len_less_than_required_hashes_suffix(self, mocker, trading_pair):
    ...
    def test_client_order_id_format_matches_expected(self, mocker, trading_pair):
...
class TestGetNewNumericClientOrderId:
    ...
    def test_numeric_client_order_id_is_integer(self, mocker):
        """Test that numeric client order ID returns an integer."""
        mock_nonce_creator = mocker.MagicMock()
        mock_nonce_creator.get_tracking_nonce.return_value = 123456789

        order_id = get_new_numeric_client_order_id(mock_nonce_creator)

        assert isinstance(order_id, int)

    def test_numeric_client_order_id_combines_host_and_nonce(self, mocker):
        """Test that numeric order ID combines host part and nonce."""
        nonce_value = 123456789
        mock_nonce_creator = mocker.MagicMock()
        mock_nonce_creator.get_tracking_nonce.return_value = nonce_value

        order_id = get_new_numeric_client_order_id(mock_nonce_creator)

        # Verify that the nonce is part of the order ID
        assert str(nonce_value) in str(order_id)

    def test_numeric_client_order_id_without_max_bit_count(self, mocker):
        ...
        order_id = get_new_numeric_client_order_id(
            mock_nonce_creator,
            max_id_bit_count=None
        )

        assert isinstance(order_id, int)
        assert order_id > 0

    def test_numeric_client_order_id_with_max_bit_count(self, mocker):
    ...
    def test_numeric_client_order_id_with_small_bit_count(self, mocker):
        """Test numeric order ID with very small bit count."""
        mock_nonce_creator = mocker.MagicMock()
        mock_nonce_creator.get_tracking_nonce.return_value = 999999999
        max_bits = 16
    ...
    def test_numeric_client_order_id_different_nonces_produce_different_ids(self, mocker):
        """Test that different nonces produce different order IDs."""
        mock_nonce_creator1 = mocker.MagicMock()
        mock_nonce_creator1.get_tracking_nonce.return_value = 111111111

        mock_nonce_creator2 = mocker.MagicMock()
        mock_nonce_creator2.get_tracking_nonce.return_value = 222222222

        order_id1 = get_new_numeric_client_order_id(mock_nonce_creator1)
        order_id2 = get_new_numeric_client_order_id(mock_nonce_creator2)
